...
export default function ArticleContent({ slug }: ArticleContentProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/articles/slug/${encodeURIComponent(slug)}`)
      ...
      .then((data) => {
        if (isMounted && data) setDbArticle(data);
      })
      .catch(console.error)
      .finally(() => {
        if (isMounted) setFetchComplete(true);
      });
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <Footer onOpenPrivacy={() => setShowPrivacy(true)} onOpenDisclaimer={() => setShowDisclaimer(true)} onSubscribe={handleSubscribe} />
      </div>
    );
  }
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground mb-6">Maaf, artikel yang Anda cari tidak tersedia.</p>
            <Link href="/">
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1">
        <div className="bg-muted/30 border-b border-border">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
            <nav className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <Link href={`/#${category?.slug}`} className="hover:text-teal-600 transition-colors">{category?.name}</Link>
              <ChevronRight className="w-4 h-4" />
        ...
        <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <motion.header initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <Badge className={`mb-4 ${colorMap[article.category] || "bg-gray-100 text-gray-800"}`}>{category?.name || article.category}</Badge>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold font-display mb-4 leading-tight">{article.title}</h1>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="flex items-center gap-1"><Clock className="w-4 h-4" />{article.readTime} menit baca</span>
                <span className="flex items-center gap-1"><Eye className="w-4 h-4" />{formatViews(article.views)} views</span>
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-2 mt-6">
              <span className="text-sm font-medium">Bagikan:</span>
              <Button variant="outline" size="sm" onClick={() => handleShare("whatsapp")} className="hover:bg-green-100 hover:text-green-600 gap-1">
                <MessageCircle className="w-4 h-4" />
                <span className="hidden sm:inline">WhatsApp</span>
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleShare("facebook")} className="hover:bg-blue-100 hover:text-blue-600 gap-1">
                <Facebook className="w-4 h-4" />
                <span className="hidden sm:inline">Facebook</span>
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleShare("twitter")} className="hover:bg-sky-100 hover:text-sky-600 gap-1">
                <Twitter className="w-4 h-4" />
                <span className="hidden sm:inline">Twitter</span>
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleShare("telegram")} className="hover:bg-blue-100 hover:text-blue-500 gap-1">
                <Share2 className="w-4 h-4" />
                <span className="hidden sm:inline">Telegram</span>
              </Button>
              <Button variant="outline" size="sm" onClick={handleCopy} className="hover:bg-teal-100 hover:text-teal-600 gap-1">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="relative aspect-video rounded-xl overflow-hidden mb-8 bg-muted">
            <Image src={article.imageUrl} alt={article.imageAlt || article.title} fill className="object-cover" priority />
          </motion.div>

          <div className="mb-8"><AdSlot position="leaderboard" /></div>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="article-content" dangerouslySetInnerHTML={{ __html: article.content }} />

          <div className="my-8"><AdSlot position="in-article" /></div>

          <div className="mt-8 pt-6 border-t border-border">
            <div className="flex flex-wrap gap-2">
              {article.tags.map((tag) => (
                <Badge key={tag} variant="secondary" className="hover:bg-teal-100 hover:text-teal-800 cursor-pointer transition-colors">#{tag}</Badge>
              ))}
            </div>
          </div>

          <div className="my-8"><AdSlot position="below-article" /></div>
          <NativeAd className="mb-8" />

          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-muted/50 rounded-xl p-6 mt-8">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mb-2">Penulis di NewsPedia</p>
                <p className="text-sm text-muted-foreground">Menulis tentang teknologi, gadget, dan tren digital terkini.</p>
              </div>
            </div>
          </motion.div>
        </article>
        ...
        <section className="bg-muted/30 py-8">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><MatchedContentAd /></div>
        </section>

        {relatedArticles.length > 0 && (
          <section className="bg-muted/30 py-12">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <h2 className="text-2xl font-bold font-display mb-6">Artikel Terkait</h2>
      ...
      <NewsletterModal open={showNewsletter} onOpenChange={setShowNewsletter} email={subscriberEmail} />
